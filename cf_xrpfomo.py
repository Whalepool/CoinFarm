...
lastprice1=round(float(decodedata['BTC_XRP']['last']),6)
lastprice='{:.7f}'.format(lastprice1)
f = open('/home/ubuntu/volume1/stakepool/teamspeakbots/lastxrp.txt', encoding='utf-8')
priorprice = f.readline().strip()
f.close()

f = open('/home/ubuntu/volume1/stakepool/teamspeakbots/lastxrp.txt', 'w+', encoding='utf-8')
...
f = open('/home/ubuntu/volume1/stakepool/teamspeakbots/xrpfomologs.txt', 'a', encoding='utf-8')
...
if pricedifffrac > 0.01: 
	...
	f = open('/home/ubuntu/volume1/stakepool/teamspeakbots/xrpfomologs.txt', 'a', encoding='utf-8')
	f.write(str(ts) + " - " + str(lastprice) + "ALERT: Ripple is FOMO'ing [b]" + direction + "[/b] by " + pricediffperc + "% to [b] in the past 5 minutes\n")
	f.close()
	time.sleep(3)
	with ts3.query.TS3Connection("158.69.115.146", 2009) as ts3conn:
		...
		if "down" in direction:
			fomots = "ALERT: [color=red][b]Ripple[/b][/color] is FOMO'ing [color=red][b]" + direction + "[/b][/color] by [color=red][b]" + str(pricediff2) + " BTC[/b][/color](" + pricediffperc + "%) to [color=red][b]" + str(lastprice) + " BTC[/b][/color] in the past 5 minutes"
		if "up" in direction:
			fomots = "ALERT: [color=green][b]Ripple[/b][/color] is FOMO'ing [color=green][b]" + direction + "[/b][/color] by [color=green][b]" + str(pricediff2) + " BTC[/b][/color](" + pricediffperc + "%) to [color=green][b]" + str(lastprice) + " BTC[/b][/color] in the past 5 minutes"
